function [param,stat,y_hat,f]=sigm_fit(x,y,fixed_params,initial_params,plot_flag)
...
stat.param=BETA';
if stat.param(3) < 0 || stat.param(2) > 1 || stat.param(2) < 0 
    [BETA,MSE,RESID] = lsqcurvefit(f,[0,0,20],x,y,[0,0.1,0],[Inf,0.9,Inf]);
end
...
y_hat = f(param(isnan(fixed_params)),x);
if plot_flag==1 
