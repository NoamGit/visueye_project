...
[xi,yi] = meshgrid(1:21,1:41);
xi = xi;
yi = yi ;
zi = exp(-(xir).^2/2/2^2-(yir).^2/2/3^2);

clear opts;
opts.tilted = true;
results = autoGaussianSurf(xi,yi,zi,opts);
disp(['angle ',num2str(angle),char(10),'angle est - ',num2str(results.angle),char(10),'angle/pi - ',num2str(results.angle*180/pi)]);

subplot(1,2,1);imagesc(zi);
subplot(1,2,2);imagesc(xi(:),yi(:),results.G);

xdata = [xi(:) yi(:)];
xdat = bsxfun(@minus,xdata,[results.x0,results.y0]);
xdat = xdat*[cos(results.angle),-sin(results.angle); sin(results.angle),cos(results.angle)];
thef = exp(-.5*(xdat.^2)*(1./[results.sigmax;results.sigmay].^2));
imagesc(xi(:),yi(:),reshape(thef,size(xi)))
%%
